package com.vango.testing.performance.viewer.controls.display
{
    ...
    public class Spinner extends Canvas
    {
        ...
        override protected function updateDisplayList(w:Number, h:Number):void {
            // safety check - make sure the spinner and its ancestors are visible
            var vis:Boolean = isVisible(this);
            if (!vis) {
                stop();
                return;
            }
            ...
            if (keepOnTop && parent) {
                ...
                var last:int = children - 1;
                if (parent.getChildIndex(this) != last) {
                    parent.setChildIndex(this, last);
                }
            }
        ...
        /**
         * Traverses up the parent hierarchy checking to make sure that all are visible.
         * If the display object is null, then false is returned.
         */
        private static function isVisible(disp:DisplayObject):Boolean {
